...
pipeline = "qtiqmmfsrc name=qmmf ! video/x-raw, format=NV12, width=640, height=480, framerate=30/1 ! videoconvert ! video/x-raw,format=BGR ! appsink drop=1"
...
class DLCPredictor:
    def __init__(self, model_path, dsp_delegate=DSP):
        ...
        self.update_timer = rospy.Timer(rospy.Duration(0.01), self.update_callback) 
